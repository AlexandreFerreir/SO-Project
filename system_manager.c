            ...
            while (j < n_alertas) {
                if(strlen((mapa_shared_var->alerta_ptr+j)->id)!=0) {
                    if(!strcmp((mapa_shared_var->alerta_ptr+j)->chave,(mapa_shared_var->chave_ptr+i)->key)){
                        if ((mapa_shared_var->chave_ptr + i)->last_value > (mapa_shared_var->alerta_ptr + j)->max || (mapa_shared_var->chave_ptr + i)->last_value < (mapa_shared_var->alerta_ptr + j)->min) {
                            msg.id_console=(mapa_shared_var->alerta_ptr + j)->id_consola;
                            sprintf(linha,"ALERT %s (%s %d TO %d) TRIGGERED",(mapa_shared_var->alerta_ptr + j)->id,(mapa_shared_var->alerta_ptr+j)->chave,(mapa_shared_var->alerta_ptr + j)->min,(mapa_shared_var->alerta_ptr + j)->max);
                            strcpy(msg.string,linha);
                            check_send=msgsnd(mqid,&msg,sizeof(Mensagem)-sizeof(long ),0);
                            if(check_send==-1){
